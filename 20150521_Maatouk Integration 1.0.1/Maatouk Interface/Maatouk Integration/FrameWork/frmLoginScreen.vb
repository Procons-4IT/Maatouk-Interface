                ...
                If oHeaderDt.Rows.Count > 0 Then
                    For Each Hrow As DataRow In oHeaderDt.Rows
                        ...
                        Else
                            ...
                            oPayment.JournalRemarks = TransactionCode
                           
                            If Hrow("CollectionDetailType") = 0 Then
                                Dim oActRecordSet As SAPbobsCOM.Recordset
                                oActRecordSet = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                                strQuery = " Select U_CashAccount From [@CASHAccounts] Where Code = '" & Hrow("CurrencyCode").ToString().Trim() & "' "
                                oActRecordSet.DoQuery(strQuery)

                                If oActRecordSet.RecordCount > 0 Then
                                    If oActRecordSet.Fields.Item(0).Value.ToString() <> "" Then
                                        oPayment.CashAccount = oActRecordSet.Fields.Item(0).Value.ToString()
                                    End If
                                End If
                            ...
                            ElseIf Hrow("CollectionDetailType") = 1 Then
                                Dim oActRecordSet As SAPbobsCOM.Recordset
                                oActRecordSet = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                                strQuery = " SELECT [U_ChecksAccount] FROM [dbo].[@CHECKSACCOUNT]  Where Code = '" & Hrow("CurrencyCode").ToString().Trim() & "' "
                                oActRecordSet.DoQuery(strQuery)

                                If oActRecordSet.RecordCount > 0 Then
                                    If oActRecordSet.Fields.Item(0).Value.ToString() <> "" Then
                                        oPayment.CheckAccount = oActRecordSet.Fields.Item(0).Value.ToString()
                                    End If
                                End If
